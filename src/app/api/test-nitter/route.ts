...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (fullResults.best_dramas.length > 0) {
      ...
      convertedStory =
        await twitterToStoryConverter.convertDramaToStory(bestDrama);
    }
    ...
    const testResults = {
      ...
      quick_check: {
        ...
        high_quality_dramas: quickDramas.filter(d => d.drama_score >= 80)
          .length,
      },

      full_aggregation: {
        sources_attempted: fullResults.results.length,
        sources_successful: fullResults.results.filter(
          r => r.source !== 'failed'
        ).length,
        ...
        conversion_rate:
          fullResults.summary.total_tweets > 0
            ? `${Math.round((fullResults.summary.drama_threads_found / fullResults.summary.total_tweets) * 100)}%`
            : '0%',
      },
