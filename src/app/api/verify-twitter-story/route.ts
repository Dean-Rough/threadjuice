...
export async function GET(request: NextRequest) {
  try {
    ...
    const postsResponse = await fetch('http://localhost:4242/api/posts?limit=5');
    const postsData = await postsResponse.json();
    
    // Find Twitter drama stories
    const twitterStories = postsData.posts.filter((post: any) => 
      post.category === 'Food Wars' && 
      post.title.includes('Twitter')
    );
