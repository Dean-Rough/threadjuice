...
async function createSimulatedTwitterStory() {
  const simulatedDramas = [
    {
      ...
      title: 'Twitter Erupts Over Whether Camping at Coffee Shops Makes You a Productivity Guru or WiFi Parasite',
      excerpt: 'A simple tweet about laptop hogging at Starbucks sparked 847 replies debating remote work ethics, capitalism, and why some people think ordering one coffee entitles them to an 8-hour office rental.',
      category: 'Work Drama',
      drama_score: 87,
      controversy: [
    ...
    },
    {
      ...
      title: 'Netflix Password Sharing Drama Reveals Which Friends Are Actually Cheap vs Strategic',
      excerpt: 'What began as a frustrated tweet about Netflix crackdowns became a 1,200-reply deep dive into friendship economics, subscription socialism, and why your college roommate still uses your HBO login.',
  ...
  const selectedDrama = simulatedDramas[Math.floor(Math.random() * simulatedDramas.length)];

  return {
    id: `simulated-twitter-${Date.now()}`,
    title: selectedDrama.title,
    slug: selectedDrama.title.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, ''),
