...
describe('ContentManagementPage', () => {
  ...
  it('renders page header', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Create, edit, and manage your posts and articles.')).toBeInTheDocument();
    });
  });
  ...
  it('renders new post button', async () => {
    ...
    await waitFor(() => {
      ...
      expect(newPostButton.closest('a')).toHaveAttribute('href', '/admin/content/new');
    });
  });
  ...
  it('displays posts after loading', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('AI Takes Over Reddit: The Drama Nobody Saw Coming')).toBeInTheDocument();
      expect(screen.getByText('Crypto Bros Discover Touch Grass: A Scientific Study')).toBeInTheDocument();
      expect(screen.getByText('Karen vs Manager: The Ultimate Showdown')).toBeInTheDocument();
    });
  });
  ...
  it('shows status badges for posts', async () => {
    ...
    await waitFor(() => {
      ...
      expect(document.querySelector('.bg-green-100.text-green-800')).toBeInTheDocument(); // Published
      expect(document.querySelector('.bg-yellow-100.text-yellow-800')).toBeInTheDocument(); // Draft
      expect(document.querySelector('.bg-blue-100.text-blue-800')).toBeInTheDocument(); // Scheduled
    });
  });
  ...
  it('has search functionality', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Crypto Bros Discover Touch Grass: A Scientific Study')).toBeInTheDocument();
      expect(screen.queryByText('AI Takes Over Reddit: The Drama Nobody Saw Coming')).not.toBeInTheDocument();
    });
  });
  ...
  it('shows bulk actions when posts are selected', async () => {
    ...
    await waitFor(() => {
      ...
      const bulkActionButtons = screen.getAllByRole('button').filter(btn => 
  ...
  it('formats dates correctly', async () => {
    ...
    await waitFor(() => {
      ...
      const dateElements = document.querySelectorAll('[class*="text-sm text-gray-600"]');
  ...
  it('shows excerpt text for posts', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('When AI started moderating r/funny, nobody expected the chaos that followed...')).toBeInTheDocument();
      expect(screen.getByText('Local crypto enthusiast shocked to discover that grass has texture...')).toBeInTheDocument();
      expect(screen.getByText('In a retail showdown for the ages, one Karen meets her match...')).toBeInTheDocument();
    });
  });
  ...
  it('shows scheduled date for scheduled posts', async () => {
    ...
    await waitFor(() => {
      ...
      const scheduledPost = screen.getByText('Karen vs Manager: The Ultimate Showdown').closest('tr');
  ...
  it('calls bulk action handlers', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Bulk publish for posts:', expect.any(Array));
