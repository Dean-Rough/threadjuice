...
describe('Enhanced PostCard', () => {
  ...
  it('should only show trending badge for high scoring posts', () => {
    const { rerender } = render(<PostCard post={{ ...mockPost, trending_score: 30 }} />);
    expect(screen.queryByTestId('trending-badge')).not.toBeInTheDocument();
  ...
  it('should have proper semantic HTML structure', () => {
    ...
    expect(time?.closest('time')).toHaveAttribute('dateTime', mockPost.created_at);
  });
  ...
  it('should have proper CSS classes for styling', () => {
    render(<PostCard post={mockPost} variant="magazine" className="custom-class" />);
  ...
  it('should include proper Font Awesome icons', () => {
    ...
    const dateIcon = screen.getByRole('article').querySelector('.far.fa-calendar-alt');
    expect(dateIcon).toBeInTheDocument();
    
    const clockIcon = screen.getByRole('article').querySelector('.far.fa-clock');
    expect(clockIcon).toBeInTheDocument();
    
    const eyeIcon = screen.getByRole('article').querySelector('.far.fa-eye');
    expect(eyeIcon).toBeInTheDocument();
    
    const shareIcon = screen.getByRole('article').querySelector('.far.fa-share-alt');
