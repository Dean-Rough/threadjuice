...
jest.mock('typewriter-effect', () => {
  return function MockTypewriter({ onInit, options }: any) {
    ...
    return <div data-testid="typewriter-mock">{options.strings?.[0] || 'Mock Typewriter'}</div>;
  };
});
...
describe('TypewriterAnimation', () => {
  it('renders with default props', () => {
    render(
      <TypewriterAnimation strings={['Hello World', 'Welcome']} />
    );
  ...
  it('handles multiple strings', () => {
    ...
    render(
      <TypewriterAnimation strings={strings} />
    );
