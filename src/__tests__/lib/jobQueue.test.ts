...
describe('JobQueue', () => {
  ...
  describe('Job Management', () => {
    ...
    it('should register and execute workers', async () => {
      ...
      expect(mockWorker).toHaveBeenCalledWith(expect.objectContaining({
        id: jobId,
        type: 'test_job',
        data: { input: 'test' }
      }));
    ...
    it('should handle job failures with retry logic', async () => {
      ...
      const jobId = await jobQueue.addJob('retry_job', { test: 'retry' }, { maxRetries: 3 });
    ...
    it('should fail jobs permanently after max retries', async () => {
      const mockWorker = jest.fn().mockRejectedValue(new Error('Persistent error'));
      jobQueue.registerWorker('fail_job', mockWorker);

      const jobId = await jobQueue.addJob('fail_job', { test: 'fail' }, { maxRetries: 2 });
