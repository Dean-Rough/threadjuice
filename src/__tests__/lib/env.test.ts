  ...
  describe('Environment Variable Validation', () => {
    it('should validate required Supabase environment variables', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.SUPABASE_SERVICE_ROLE_KEY).toBe(mockEnv.SUPABASE_SERVICE_ROLE_KEY);
      }
    });
    ...
    it('should validate Clerk authentication keys', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.CLERK_PUBLISHABLE_KEY).toBe(mockEnv.CLERK_PUBLISHABLE_KEY);
      }
    });
    ...
    it('should validate Reddit API configuration', () => {
      ...
      if (result.success) {
        ...
        expect(result.data.REDDIT_CLIENT_SECRET).toBe(mockEnv.REDDIT_CLIENT_SECRET);
    ...
    it('should reject weak or placeholder values', () => {
      const securitySchema = z.object({
        API_KEY: z.string().refine(
          (val) => !['test', 'placeholder', 'changeme'].includes(val.toLowerCase()),
      ...
      const goodResult = securitySchema.safeParse({ API_KEY: 'sk-real-api-key-12345' });
