...
} from '@/lib/contentQualityChecker';
  ...
  while ((match = categoryPattern.exec(content)) !== null) {
    ...
    const overlaps = replacements.some(
      r =>
        ...
        (match!.index + match![0].length > r.start &&
          match!.index + match![0].length <= r.end)
    );
...
export default function SimplePostDetail({
  ...
  const [contentQuality, setContentQuality] =
    useState<ContentQualityMetrics | null>(null);
  const [adaptiveSections, setAdaptiveSections] = useState<any[]>([]);
  const [emotionalAnalysis, setEmotionalAnalysis] = useState<
    (EmotionalAnalysis & { sectionIndex: number; sectionType?: string })[]
  >([]);
  const [metaphorInsight, setMetaphorInsight] =
    useState<MetaphorInsight | null>(null);
  ...
  const getGifCaption = (emotion: EmotionalAnalysis): string => {
    ...
    const emotionCaptions = captions[emotion.emotion] || [
      'Everyone right now:',
    ];
  ...
  const enhanceStoryWithGifs = useCallback(
    async (sections: any[], emotions: (EmotionalAnalysis & { sectionIndex: number; sectionType?: string })[]): Promise<any[]> => {
  ...
  const getInsertionContent = (
    ...
    if (insertionType < 4) {
      // Pull quote
      return (
        <blockquote className='my-6 rounded-r-lg border-l-4 border-orange-500 bg-muted/30 py-4 pl-6'>
      ...
      return (
        <div className='jaunty-cutout mx-4 rounded-lg border border-border bg-card p-6'>
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              <p className='mb-3 text-foreground'>{tweet.content}</p>
      ...
      if (
        emotion &&
        emotion.giffSearchTerms &&
        emotion.giffSearchTerms.length > 0
      ) {
        ...
        if (postData.content?.sections) {
          try {
            const quality = contentQualityChecker.analyzeContent({
              ...
              content: postData.content.sections
                .map((s: any) => s.content)
                .join('\n\n'),
            ...
            const adaptedSections = adaptSectionsForQuality(
              postData.content.sections,
              quality
            );
            setAdaptiveSections(adaptedSections);

            // Analyze emotions and enhance story with GIF reactions
            const emotions = await analyzeEmotionsForSections(
              postData.content.sections,
              {
                category: postData.category,
            ...
            if (quality.passesPublishingThreshold) {
              ...
              const enhancedSections = await enhanceStoryWithGifs(
                adaptedSections,
                emotions
              );
        ...
        if (response.ok) {
          ...
          const related = allPosts.posts
            ...
            .sort((a: any, b: any) => {
              ...
              const aSharedTags =
                tags.length > 0 && a.tags
                  ? a.tags.filter((tag: string) => tags.includes(tag)).length
                  : 0;
              const bSharedTags =
                tags.length > 0 && b.tags
                  ? b.tags.filter((tag: string) => tags.includes(tag)).length
                  : 0;
              if (aSharedTags !== bSharedTags) return bSharedTags - aSharedTags;

              // Finally by trending score
              return (
                (b.trending_score || b.viral_score || 0) -
                (a.trending_score || a.viral_score || 0)
              );
            })
  ...
  const calculateDynamicReadingTime = (
    sections: any[],
    quality: ContentQualityMetrics | null
  ): number => {
    ...
    sections.forEach(section => {
      if (section.content) {
        ...
        if (section.enhanced || quality.qualityTier === 'premium') {
          ...
          section.type === 'twitter-conversation' ||
          section.type === 'comments-1'
        ) {
    ...
    const readingTime = Math.ceil(
      (totalWords / baseWordsPerMinute) * qualityMultiplier
    );
  ...
  const analyzeEmotionsForSections = async (
  ...
  ): Promise<(EmotionalAnalysis & { sectionIndex: number; sectionType?: string })[]> => {
    const emotions: (EmotionalAnalysis & { sectionIndex: number; sectionType?: string })[] = [];
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (
        section.content &&
        section.type &&
        ['describe-1', 'describe-2', 'quotes', 'discussion', 'outro'].includes(
          section.type
        )
      ) {
                ...
                {media.confidence < 0.8 && (
                  <p className='mt-1 text-xs text-muted-foreground'>
                    <span className='italic'>
                      Related video - exact match unavailable
                    </span>
                  </p>
                )}
              </div>
          ...
          <div key={index} className='media-embed-section my-12'>
            ...
            {media.confidence < 0.8 && (
              <p className='mt-2 text-center text-xs text-muted-foreground'>
                <span className='italic'>
                  Related tweet - exact match unavailable
                </span>
              </p>
            )}
          </div>
        ...
        return (
          <div key={index} className='media-embed-section my-12'>
            ...
            {media.confidence < 0.8 && (
              <p className='mt-2 text-center text-xs text-muted-foreground'>
                <span className='italic'>
                  Related TikTok - exact match unavailable
                </span>
              </p>
            )}
          </div>
    ...
    switch (section.type) {
            ...
            <div className='overflow-hidden rounded-lg border border-border'>
              <Image
                src={
                  ...
                  `/assets/img/lifestyle/life_style0${(index % 9) + 1}.jpg`
                }
                alt={section.content}
                width={800}
                height={400}
        ...
        return (
          <div key={index} className='hero-image-section mb-8'>
            <Image
              src={
                section.metadata?.imageUrl ||
                `/assets/img/blog/blog${Math.floor(Math.random() * 15) + 1}.jpg`
              }
          ...
          <div key={index} className='embedded-image-section my-12'>
            ...
            <div className='overflow-hidden rounded-lg border border-border bg-card'>
              {section.metadata?.imageUrl ? (
                ...
                <div className='flex h-64 items-center justify-center bg-gradient-to-br from-orange-100 to-orange-200'>
      ...
      case 'describe':
        return (
          <div
            key={index}
            className={`description-section mb-8 ${section.enhanced ? 'enhanced-content' : ''}`}
          >
            {section.title && (
              <h3
              ...
              {section.content
                ...
                .map((paragraph: string, pIndex: number) => {
                  ...
                  if (
                    ...
                    const sectionEmotion = emotionalAnalysis.find(
                      e => e.sectionIndex === index
                    );

                    // Insert pull quote, tweet, or GIF
                    const insertionContent = getInsertionContent(
                      pIndex,
                      sectionEmotion
                    );
                    if (insertionContent) {
            ...
            {section.metadata?.comments &&
              (section.metadata.platform === 'twitter' ? (
                <TwitterConversation
                  ...
                  conversation={section.metadata.comments.map(
                    (comment: any, index: number) => ({
                      ...
                      retweets:
                        comment.retweets ||
                        Math.floor((comment.score || 0) * 0.3),
                ...
                <div className='reddit-comments space-y-4'>
                  {section.metadata.comments.map(
                    (comment: any, index: number) => (
                      <div
                        key={index}
                        className='rounded-lg border-l-4 border-orange-500 bg-muted/50 p-4'
                      >
                        <div className='flex items-start gap-3'>
                          <div className='flex flex-col items-center'>
                            <button className='text-muted-foreground transition-colors hover:text-orange-500'>
      ...
      case 'terry_corner':
        return (
          <div
            key={index}
            className='terry-corner-section my-12 rounded-lg border-2 border-orange-500 bg-orange-500/10 p-6'
          >
              ...
              <div className='relative aspect-video bg-black'>
                {section.metadata?.thumbnail ? (
                  ...
                  <div className='flex h-full w-full items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900'>
                    <span className='text-lg text-white'>📹 Video Content</span>
                  </div>
                )}
            ...
            return (
              <div key={index} className='tiktok-link-section my-8 text-center'>
                <a
                  ...
                  className='inline-flex items-center gap-3 rounded-full bg-orange-500 px-6 py-3 font-semibold text-white transition-colors hover:bg-orange-600'
                >
        ...
        <div className='grid gap-8 lg:grid-cols-4'>
          ...
          <div className='lg:col-span-3'>
            <article className='max-w-none'>
                  ...
                  <div className='flex items-center space-x-4'>
                    ...
                    <div>
                      ...
                      <div className='text-sm text-muted-foreground'>
                        ...
                        {contentQuality
                          ? calculateDynamicReadingTime(
                              adaptiveSections.length > 0
                                ? adaptiveSections
                ...
                {(post.content?.sections || []).map(
                  (section: any, index: number) => (
                    <React.Fragment key={index}>
                      ...
                      {index ===
                        Math.floor(
                          (adaptiveSections.length ||
                            post.content?.sections?.length ||
                            0) / 2
                        ) &&
                        index > 2 &&
                        contentQuality?.qualityTier === 'premium' && (
              ...
              {showRelated && relatedStories.length > 0 && (
                <div className='more-stories-section mt-16 border-t border-border pt-12'>
                  ...
                  <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
                    {relatedStories.map(story => (
                      ...
                      >
                        <article className='h-full overflow-hidden rounded-lg border border-border bg-card transition-all hover:border-orange-500/50 hover:shadow-lg'>
                          {/* Featured Image */}
                          ...
                          {/* Card Content */}
                          <div className='p-5'>
                            <h3 className='mb-3 line-clamp-2 text-lg font-bold text-foreground transition-colors group-hover:text-orange-600'>
                              {story.title}
                            </h3>
