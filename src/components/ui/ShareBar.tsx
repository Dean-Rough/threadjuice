...
export const ShareBar: React.FC<ShareBarProps> = ({
  ...
  const shareBarClasses = `sarsa-share-bar sarsa-share-bar--${variant} ${className}`.trim();

  const socialPlatforms = [
    { key: 'facebook', icon: 'fab fa-facebook-f', label: 'Facebook', color: '#1877f2' },
    { key: 'twitter', icon: 'fab fa-twitter', label: 'Twitter', color: '#1da1f2' },
    { key: 'linkedin', icon: 'fab fa-linkedin-in', label: 'LinkedIn', color: '#0077b5' },
    { key: 'reddit', icon: 'fab fa-reddit-alien', label: 'Reddit', color: '#ff4500' },
    ...
    <div className="sarsa-share-bar__container">
      {shareCount !== undefined && (
        <div className='sarsa-share-bar__count'>
          <span className='sarsa-share-bar__count-number'>{shareCount.toLocaleString()}</span>
