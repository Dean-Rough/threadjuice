...
export class StoryRewriter {
  ...
  private readonly TARGET_SCORE = 0.7;
  ...
  async rewriteStory(
    ...
    console.log(
      `📊 Current score: ${Math.round(currentQuality.overallQuality * 100)}%`
    );
    console.log(`🎯 Target score: ${Math.round(this.TARGET_SCORE * 100)}%`);

    while (
      attempt <= maxAttempts &&
      currentQuality.overallQuality < this.TARGET_SCORE
    ) {
      try {
        ...
        console.log(
          `🔍 Addressing ${highPrioritySuggestions.length} high priority issues:`
        );
        highPrioritySuggestions.forEach((suggestion, index) => {
          console.log(
            `  ${index + 1}. ${suggestion.category}: ${suggestion.issue}`
          );
        });

        // Apply improvements
        const improvedStory = await this.applyImprovements(
          currentStory,
        ...
        const newQuality = contentQualityChecker.analyzeContent({
          ...
          content:
            improvedStory.content?.sections
              ?.map((s: any) => s.content)
              .join('\n\n') || '',
        ...
        console.log(
          `📈 New score: ${Math.round(newQuality.overallQuality * 100)}% (was ${Math.round(currentQuality.overallQuality * 100)}%)`
        );

        // Track improvements made
        const improvementsMade = highPrioritySuggestions.map(
          s => `${s.category}: ${s.suggestion.substring(0, 100)}...`
        );
        ...
        if (newQuality.overallQuality <= currentQuality.overallQuality) {
          ...
          errors.push(
            `Attempt ${attempt}: No quality improvement despite changes`
          );
        }
        ...
        if (currentQuality.overallQuality >= this.TARGET_SCORE) {
          console.log(
            `✅ Target score achieved: ${Math.round(currentQuality.overallQuality * 100)}%`
          );
          break;
        }
      } catch (error) {
  ...
  private async applyImprovements(
    ...
    for (const suggestion of suggestions) {
      switch (suggestion.category) {
        case 'readability':
          improvedStory = await this.improveReadability(
            improvedStory,
            suggestion
          );
          break;
        case 'engagement':
          improvedStory = await this.improveEngagement(
            improvedStory,
            suggestion
          );
          break;
        case 'narrative':
          improvedStory = await this.improveNarrative(
            improvedStory,
            suggestion
          );
          break;
        case 'terry_voice':
          improvedStory = await this.improveTerryVoice(
            improvedStory,
            suggestion
          );
          break;
        case 'originality':
          improvedStory = await this.improveOriginality(
            improvedStory,
            suggestion
          );
          break;
  ...
  private async improveEngagement(
    story: any,
    suggestion: ImprovementSuggestion
  ): Promise<any> {
    ...
    const hasEmotionalHook = emotionalWords.some(word =>
  ...
  private async improveNarrative(
    story: any,
    suggestion: ImprovementSuggestion
  ): Promise<any> {
    ...
    if (suggestion.issue.includes('quotes')) {
      ...
      const insertIndex =
        story.content.sections.findIndex((s: any) => s.type === 'describe-1') +
        1;
      story.content.sections.splice(insertIndex, 0, quoteSection);
    }

    if (
      suggestion.issue.includes('comments') ||
      suggestion.issue.includes('reactions')
    ) {
      ...
      // Insert before outro
      const outroIndex = story.content.sections.findIndex(
        (s: any) => s.type === 'outro'
      );
      if (outroIndex > -1) {
    ...
    if (
      suggestion.issue.includes('discussion') ||
      suggestion.issue.includes('bigger picture')
    ) {
  ...
  private async improveTerryVoice(
    story: any,
    suggestion: ImprovementSuggestion
  ): Promise<any> {
  ...
  private async improveOriginality(
    story: any,
    suggestion: ImprovementSuggestion
  ): Promise<any> {
    ...
    if (story.content?.sections) {
      const discussionSection = story.content.sections.find(
        (s: any) => s.type === 'discussion'
      );
    ...
    if (story.excerpt) {
      story.excerpt = story.excerpt.replace(
        /.*explained.*/,
        'A perfect case study in how trust becomes vulnerability in the digital age.'
      );
    }
  ...
  shouldRewrite(story: any): {
    ...
    const qualityMetrics = contentQualityChecker.analyzeContent({
      title: story.title,
      content:
        story.content?.sections?.map((s: any) => s.content).join('\n\n') || '',
