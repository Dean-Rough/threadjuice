...
const KNOWN_ENTITIES = {
  celebrities: [
    'kardashian', 'jenner', 'swift', 'beyonce', 'kanye', 'kim kardashian',
    'judge judy', 'oprah', 'elon musk', 'bezos', 'gates'
  ],
  places: [
    'bermuda', 'switzerland', 'paris', 'london', 'new york', 'california',
    'tokyo', 'rome', 'berlin', 'amsterdam', 'barcelona'
  ],
  brands: [
    'google', 'apple', 'microsoft', 'tesla', 'netflix', 'amazon',
    'facebook', 'instagram', 'tiktok', 'uber', 'airbnb'
  ],
  landmarks: [
    'eiffel tower', 'statue of liberty', 'big ben', 'golden gate bridge',
...
export class IntelligentImageRouter {
   ...
   */
  analyzeContent(title: string, content: string, category: string): ContentAnalysis {
   ...
   */
  private extractVisualConcepts(title: string, content: string, category: string): string[] {
    ...
    const visualKeywords = {
      // Legal/courtroom
      legal: ['courtroom', 'judge', 'jury', 'lawyer', 'gavel', 'courthouse', 'legal documents'],
      
      // Technology
      technology: ['computer', 'phone', 'app', 'screen', 'office', 'startup', 'coding', 'meeting room'],
      
      // Travel
      travel: ['car', 'road', 'map', 'airport', 'hotel', 'mountain', 'parking lot', 'gps', 'luggage'],
      
      // Politics
      politics: ['office', 'meeting', 'debate', 'podium', 'government building', 'press conference'],
      
      // Relationships/AITA
      relationships: ['home', 'apartment', 'kitchen', 'bedroom', 'restaurant', 'couple', 'family'],
      
      // Work stories
      'work-stories': ['office', 'desk', 'meeting room', 'computer', 'conference table', 'workplace'],
      
      // Food/restaurants
      food: ['restaurant', 'pizza', 'kitchen', 'delivery', 'food', 'chef', 'dining']
    }
    
    // Get category-specific keywords
    const categoryKeywords = visualKeywords[category as keyof typeof visualKeywords] || []
    ...
    const objectRegex = /(?:a|an|the)\s+([a-z]+(?:\s+[a-z]+)*?)(?:\s+(?:was|is|were|are|in|on|at|with))/g
    let match
    while ((match = objectRegex.exec(text)) !== null) {
      const object = match[1].trim()
      if (object.length > 2 && !['way', 'time', 'case', 'thing', 'person'].includes(object)) {
  ...
  private determineSetting(text: string, category: string): string {
    ...
    return categoryToSetting[category as keyof typeof categoryToSetting] || 'general'
  }
  ...
  private determineMood(text: string, category: string): string {
    const moodKeywords = {
      'dramatic': ['chaos', 'disaster', 'crisis', 'drama', 'explosive', 'tension'],
   ...
   */
  generateStockImagePrompt(title: string, content: string, category: string): string {
    ...
    if (categoryModifiers[category as keyof typeof categoryModifiers]) {
      components.push(categoryModifiers[category as keyof typeof categoryModifiers])
    }
