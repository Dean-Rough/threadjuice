  ...
  private async processJobs(): Promise<void> {
    ...
    for (const job of pendingJobs) {
      ...
      if (!this.workers.has(job.type)) {
        logger.error('No worker registered for job type', { jobType: job.type, jobId: job.id });
        this.updateJobStatus(job.id, 'failed', 0, undefined, 'No worker registered');
        continue;
      }
  ...
  private async processJob(job: Job): Promise<void> {
    ...
    try {
      ...
      if (job.retryCount < job.maxRetries) {
        ...
        logger.info('Job queued for retry', { jobId: job.id, retryCount: job.retryCount });
      } else {
        this.updateJobStatus(job.id, 'failed', job.progress, undefined, errorMessage);
        logger.error('Job failed permanently', { jobId: job.id, maxRetries: job.maxRetries });
      }
    } finally {
