...
class TwitterRateLimiter {
  ...
  canMakeCall(): { allowed: boolean; reason?: string; stats: any } {
    ...
    const monthCalls = this.callLog.filter(
      call => call.timestamp >= monthStart
    );
  ...
  getUsageStats() {
    ...
    return {
      monthly: {
        ...
        success_rate:
          monthCalls.length > 0
            ? (successfulCalls.length / monthCalls.length) * 100
            : 0,
      },
      daily: {
  ...
  getOptimalCallTiming() {
    ...
    const recommendedDailyLimit = Math.floor(
      stats.monthly.remaining / Math.max(daysLeftInMonth, 1)
    );

    return {
      calls_remaining_this_month: stats.monthly.remaining,
      days_left_in_month: daysLeftInMonth,
      recommended_daily_limit: Math.min(
  ...
  shouldRunDramaDetection(): { should_run: boolean; reason: string } {
    ...
    if (
      timing.calls_remaining_this_month < 10 &&
      timing.days_left_in_month > 5
    ) {
      return {
        should_run: false,
