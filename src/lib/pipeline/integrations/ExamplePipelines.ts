...
import { GenerationStage, RedditGeneration, AIGeneration, TwitterGeneration } from '../stages/GenerationStage';
 ...
 */
export function createRedditPipeline(options: {
  subreddit: string;
  personaId: string;
  minScore?: number;
  ...
  return new PipelineBuilder()
    .add(RedditSource(options.subreddit, {
      sort: 'hot',
      limit: 10,
      minScore: options.minScore || 100,
    }))
    .add(new AnalysisStage({
    ...
    .add(RedditGeneration(options.personaId, {
...
export function createAIPipeline(options: {
  ...
  return new PipelineBuilder()
    ...
    .add(AIGeneration(options.persona, {
      temperature: 0.8,
      maxTokens: 3000,
    }))
 ...
 */
export function createTwitterPipeline(options: {
  personaId: string;
...
export function createQuickRedditPipeline(subreddit: string) {
  return new PipelineBuilder()
    ...
    .add(RedditGeneration('the-terry', {
 ...
 */
export async function createMultiSourcePipeline(personaId: string = 'the-terry') {
  ...
  if (services.reddit.available) {
    pipelines.push(createRedditPipeline({
      subreddit: 'tifu',
      personaId,
    }));
  }
  ...
  pipelines.push(createAIPipeline({
    category: 'general',
    persona: personaId,
  }));
