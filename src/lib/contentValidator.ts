...
export class ContentValidator {
  ...
  private checkSafety(content: string): string[] {
    ...
    for (const pattern of this.prohibitedPatterns) {
      if (pattern.test(content)) {
        issues.push(
          'Content contains potentially harmful or private information'
        );
        break;
      }
    }
    ...
    if (
      sentences.length > 10 &&
      uniqueSentences.size / sentences.length < 0.7
    ) {
  ...
  private checkQuality(
    ...
    if (wordCount < this.qualityIndicators.minWordCount) {
      issues.push(
        `Content is too short (${wordCount} words, minimum ${this.qualityIndicators.minWordCount})`
      );
    } else if (wordCount > this.qualityIndicators.maxWordCount) {
      warnings.push(
        `Content is quite long (${wordCount} words, recommended maximum ${this.qualityIndicators.maxWordCount})`
      );
    }
  ...
  private checkStructure(content: string): string[] {
    ...
    const hasHeadings =
      /^#{1,6}\s+/m.test(content) || /<h[1-6]>/i.test(content);
  ...
  getMetrics(content: string, title?: string): ContentMetrics {
    ...
    return {
      ...
      qualityIndicators: {
        ...
        isEngaging:
          readabilityScore > 60 && wordCount >= 200 && wordCount <= 1500,
      },
    };
  }
}
