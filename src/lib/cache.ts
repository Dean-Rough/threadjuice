...
export function setCacheHeaders(
  ...
  const { staleWhileRevalidate = duration / 2, mustRevalidate = false, noCache = false } = options;
...
export class RedisCache {
  ...
  async set(key: string, data: any, ttl: number = CACHE_DURATIONS.MEDIUM): Promise<void> {
...
export function setEdgeCacheHeaders(
  ...
  response.headers.set('Cache-Control', `s-maxage=${duration}, stale-while-revalidate`);
