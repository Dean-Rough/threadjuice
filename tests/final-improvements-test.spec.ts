...
test('Final improvements check - images, icons, hero, card styling', async ({
  page,
}) => {
  await page.goto('/');
  ...
  const heroHeadline = await page
    .locator('text=/restaurant charged me.*emotional labor/i')
    .count();
  ...
  const trendingIcon = await page
    .locator('svg')
    .filter({ has: page.locator('[data-testid="trending-up"]') })
    .count();
  const trophyIcon = await page
    .locator('svg')
    .filter({ has: page.locator('[data-testid="trophy"]') })
    .count();
  ...
  const writerNames = await page
    .locator('text=/Alex Chen|Sam Rivera|Jordan Blake|Casey Morgan/')
    .count();
  // console.log('Writer names (should be 0):', writerNames);

  // Check for new card text styling
  const cardTitles = await page
    .locator('.tracking-tight.text-med.font-bold')
    .count();
  ...
  const modernFeatures =
    heroHeadline > 0 && cardImages > 0 && orangeBadges > 0 && writerNames === 0;
