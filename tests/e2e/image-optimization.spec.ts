...
test.describe('Image Optimization E2E Tests', () => {
  test('should load optimized images on homepage', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="optimized-image"]', { timeout: 10000 });
  ...
  test('should have priority images for above-the-fold content', async ({ page }) => {
    await page.goto('/');
    
    // Check for priority images (featured content)
    const priorityImages = await page.locator('img[fetchpriority="high"], img[priority]').all();
  ...
  test('should load images in different layouts correctly', async ({ page }) => {
    ...
    for (const image of gridImages.slice(0, 3)) { // Test first 3 images
      await expect(image).toBeVisible();
    }
  });

  test('should optimize images for different viewport sizes', async ({ page }) => {
