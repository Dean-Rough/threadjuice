...
test.describe('Critical User Flows', () => {
  ...
  test.describe('Homepage and Navigation', () => {
    ...
    test('should navigate between main pages', async ({ page }) => {
      ...
      const personasLink = page.locator('a[href*="/personas"], a:has-text("Personas"), a:has-text("Writers")').first();
  ...
  test.describe('Content Discovery', () => {
    test('should browse and filter content', async ({ page }) => {
      ...
      const categoryFilter = page.locator('select[id*="category"], .category-filter select, [data-testid="category-filter"]').first();
    ...
    test('should view individual post', async ({ page }) => {
      ...
      const postLink = page.locator('a[href*="/posts/"], a[href*="/blog/"], .post-card a, article a').first();
      ...
      if (await postLink.isVisible()) {
        ...
        await expect(page.locator('h1, .post-title, .article-title')).toBeVisible();
        await expect(page.locator('.post-content, .article-content, main')).toBeVisible();
        
        // Check for author/persona information
        const authorInfo = page.locator('.author, .persona, [data-testid="author"]').first();
  ...
  test.describe('Interactive Features', () => {
    test('should interact with quiz if available', async ({ page }) => {
      ...
      const quizLink = page.locator('a:has-text("Quiz"), a[href*="quiz"], .quiz-link').first();
      if (await quizLink.isVisible()) {
        await quizLink.click();
        
        // Check if quiz loads
        await expect(page.locator('.quiz, [data-testid="quiz"], .quiz-container')).toBeVisible();
        
        // Try to interact with first question if available
        const firstOption = page.locator('input[type="radio"], .quiz-option, .question input').first();
        if (await firstOption.isVisible()) {
          await firstOption.click();
          
          // Look for submit or next button
          const submitButton = page.locator('button:has-text("Submit"), button:has-text("Next"), .quiz-submit').first();
    ...
    test('should handle theme switching if available', async ({ page }) => {
      const themeToggle = page.locator('[data-testid="theme-toggle"], .theme-switch, button:has-text("Dark"), button:has-text("Light")').first();
  ...
  test.describe('Performance and Accessibility', () => {
    test('should load pages within acceptable time', async ({ page }) => {
      ...
      try {
        const fcp = await Promise.race([fcpPromise, new Promise(resolve => setTimeout(() => resolve(null), 3000))]);
        if (fcp) {
    ...
    test('should handle errors gracefully', async ({ page }) => {
      ...
      if (statusCode === 404) {
        ...
        await expect(page.locator('h1:has-text("404"), h1:has-text("Not Found"), .error-404')).toBeVisible();
      }
      
      // Test navigation still works from error page
      const homeLink = page.locator('a[href="/"], a:has-text("Home"), .logo a').first();
  ...
  test.describe('Search and Filtering', () => {
    test('should search content effectively', async ({ page }) => {
      ...
      if (await searchInput.isVisible()) {
        ...
        const hasResults = await page.locator('.post-card, .search-result, article').first().isVisible();
        const noResults = await page.locator(':has-text("No results"), :has-text("No posts"), .no-results').first().isVisible();
  ...
  test.describe('Social Features', () => {
    test('should display social sharing options', async ({ page }) => {
      ...
      const postLink = page.locator('a[href*="/posts/"], a[href*="/blog/"]').first();
      if (await postLink.isVisible()) {
        await postLink.click();
        await page.waitForLoadState('networkidle');
        
        // Look for social sharing buttons
        const shareButtons = page.locator('.share, .social-share, [data-testid="share"]').first();
        if (await shareButtons.isVisible()) {
          await expect(shareButtons).toBeVisible();
          
          // Check for common social platforms
          const twitterShare = page.locator('a[href*="twitter.com"], a:has-text("Twitter"), .twitter-share').first();
          const facebookShare = page.locator('a[href*="facebook.com"], a:has-text("Facebook"), .facebook-share').first();
          
          if (await twitterShare.isVisible() || await facebookShare.isVisible()) {
...
test.describe('Authentication Flows', () => {
  test('should handle sign in/sign up flows', async ({ page }) => {
    ...
    const signInButton = page.locator('a:has-text("Sign in"), a:has-text("Login"), .sign-in, [data-testid="sign-in"]').first();
    
    if (await signInButton.isVisible()) {
      await signInButton.click();
      
      // Should navigate to sign in page or show modal
      const signInForm = page.locator('form, .sign-in-form, [data-testid="auth-form"]').first();
