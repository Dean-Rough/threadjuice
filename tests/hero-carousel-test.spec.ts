...
test('Hero carousel with animations and navigation', async ({ page }) => {
  ...
  const heroSection = await page
    .locator('section.relative.overflow-hidden')
    .count();
  ...
  const navDots = await page
    .locator('div.absolute.bottom-8.right-8 button')
    .count();
  ...
  const heroTitle = await page
    .locator('h1.text-4xl, h1.text-6xl, h1.text-7xl')
    .count();
  ...
  const heroFeatures =
    heroSection > 0 && navDots >= 3 && heroTitle > 0 && stats > 0;
