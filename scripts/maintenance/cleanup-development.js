...
const CLEANUP_PATTERNS = {
  ...
  // Backup files
  backupFiles: /(pre-comment-fix-backup|pre-emoji-removal-backup|pre-quiz-removal-backup|backup\.json)$/,
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        } else if (CLEANUP_PATTERNS.testScripts.test(file) && file.endsWith('.js')) {
...
async function createBackupDirectory() {
  const backupDir = path.join(process.cwd(), 'archive', 'development-cleanup-' + Date.now());
...
async function main() {
  ...
  try {
    ...
    const totalFiles = Object.values(toCleanup).reduce((sum, arr) => sum + arr.length, 0);
    // console.log(`\n📋 Cleanup Summary:`);
