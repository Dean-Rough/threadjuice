...
async function generateStory(topic: string, persona: string): Promise<StoryData | null> {
  try {
  ...
  "tags": ["tag1", "tag2", "tag3"]
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a viral content writer for ThreadJuice, creating engaging stories from Reddit-style drama.'
        },
        {
          role: 'user',
          content: prompt
...
async function main() {
  console.log('🚀 Starting minimal batch ingestion of 10 AI-generated stories...\n');
