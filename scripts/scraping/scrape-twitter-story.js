...
async function scrapeTwitterThread(startUrl) {
  ...
  try {
    ...
    const threadTweets =
      threadSearch.data && threadSearch.data.length > 0
        ? threadSearch.data
        : [mainTweet.data];
    const author = mainTweet.includes.users[0];
    const allMedia = [
      ...(mainTweet.includes?.media || []),
      ...(threadSearch.includes?.media || []),
    ];
    const mediaMap = new Map(allMedia.map(m => [m.media_key, m]));

    console.log(
      `✅ Found thread with ${threadTweets.length} tweets by @${author.username}`
    );
...
async function scrapeTwitterDrama(urls) {
  ...
  for (const url of urls) {
    try {
      ...
      console.log(
        `✅ Found drama with ${conversation.length} tweets and ${participants.size} participants`
      );
    } catch (error) {
  ...
  return {
    ...
    totalEngagement: conversation.reduce(
      (sum, t) => sum + t.likes + t.retweets,
      0
    ),
  };
...
function convertThreadToStory(threadData) {
  ...
  sections.push({
    ...
    content:
      "Another day, another Twitter thread that could've been a blog post. But here we are, consuming content 280 characters at a time like digital crackheads.",
  });
...
function convertDramaToStory(dramaData) {
  ...
  const mediaFound = dramaData.conversation.filter(
  ...
  sections.push({
    ...
    title: "The Terry's Take",
    content:
      "Watching grown adults have public meltdowns on Twitter is like watching a car crash in slow motion. You know you should look away, but you just can't. Pass the popcorn.",
  });
...
async function main() {
  ...
  try {
    ...
    if (mode === 'thread') {
      ...
      const dramaUrls = args
        .slice(1)
        .filter(url => url.includes('twitter.com'));
      if (dramaUrls.length === 0) {
        console.error(
          '❌ Please provide at least one Twitter URL for the drama'
        );
        process.exit(1);
      }
      ...
      console.log(
        `✅ Scraped drama with ${dramaData.conversation.length} tweets`
      );
