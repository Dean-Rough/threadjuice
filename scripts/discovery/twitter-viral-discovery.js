...
async function searchViralTweets(client, query, maxResults = 100) {
  try {
    ...
    console.error(
      `❌ Search failed for query "${query.slice(0, 30)}...":`,
      error.message
    );
    return [];
  }
 ...
 */
async function getViralThreads(
  client,
  authorId,
  conversationId,
  maxTweets = 20
...
function filterQualityTweets(tweets) {
  return tweets.filter(tweet => {
    ...
    if (tweet.text.startsWith('@') && tweet.engagement_score < 5000)
      return false;
...
async function discoverViralContent(options = {}) {
  ...
  console.log(
    `📊 Min engagement: ${minEngagement}, Include threads: ${includeThreads}`
  );
  ...
  for (const item of content) {
    try {
      if (item.type === 'thread') {
        ...
        console.log(
          `   👤 @${item.mainTweet.author_username} | 🔥 ${item.totalEngagement} engagement`
        );

        // Use the Twitter scraper for threads
        const { execSync } = await import('child_process');
        execSync(
          `node scripts/scraping/scrape-twitter-story.js thread ${item.url}`,
          {
            stdio: 'inherit',
          }
        );
        ...
        console.log(
          `   👤 @${item.tweet.author_username} | 🔥 ${item.tweet.engagement_score} engagement`
        );
        ...
        if (item.tweet.metrics.quote_count > 100) {
          const { execSync } = await import('child_process');
          execSync(
            `node scripts/scraping/scrape-twitter-story.js drama ${item.url}`,
            {
              stdio: 'inherit',
            }
          );
  ...
  try {
    if (command === 'discover') {
      ...
      if (args.includes('--dry-run')) {
        ...
        content.forEach((item, i) => {
          if (item.type === 'thread') {
            ...
            console.log(
              `   Engagement: ${item.totalEngagement} | Tweets: ${item.threadTweets.length}`
            );
          } else {
            console.log(
      ...
      trends[0].trends.slice(0, 20).forEach((trend, i) => {
        console.log(
          `${i + 1}. ${trend.name} (${trend.tweet_volume || 'N/A'} tweets)`
        );
      });
