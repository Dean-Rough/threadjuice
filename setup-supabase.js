...
async function setupDatabase() {
  ...
  try {
    ...
    const { error: personasError } = await supabase.rpc(
      'create_personas_table',
      {}
    );
    console.log(
      'Personas table result:',
      personasError ? personasError.message : '✅ Success'
    );
  } catch (error) {
...
async function testAndSetup() {
  ...
  try {
    ...
    console.log(
      '📋 Manual setup required. Copy schema.sql to Supabase SQL Editor.'
    );
