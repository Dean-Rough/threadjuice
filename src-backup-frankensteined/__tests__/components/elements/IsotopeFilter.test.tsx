...
describe('IsotopeFilter', () => {
  ...
  it('applies default active filter to "All" button', () => {
    ...
    expect(screen.getByRole('button', { name: 'Technology' })).not.toHaveClass(
      'active'
    );
  });
  ...
  it('initializes Isotope on mount', async () => {
    ...
    render(
      <IsotopeFilter filters={['technology']}>{mockChildren}</IsotopeFilter>
    );
  ...
  it('includes sizing elements for masonry layout', () => {
    ...
    expect(
      container.querySelector('.isotope-gutter-sizer')
    ).toBeInTheDocument();
  });
