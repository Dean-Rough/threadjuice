...
async function clearPosts() {
  ...
  try {
    ...
    for (const post of existingPosts) {
      ...
      if (deleteError) {
        console.log(`❌ Failed to delete "${post.title}": ${deleteError.message}`);
      } else {
        deletedCount++;
    ...
    console.log(`✅ Successfully deleted ${deletedCount} out of ${existingPosts.length} posts from database!`);
